...
def plot_basic_linear_regression(X, Y, title="title", x_axis="x_axis", y_axis="y_axis"):
    alpha = 100.0
    res = 1
    theta = [[0.0]]* (X.shape[1])
    print(f"theta = {theta}")
    while res > 0:
        ...
        print(f"res = {res}")
    mlr = MLR(theta)
    mlr.fit_(X, Y, alpha=alpha, n_cycle=1000)
    data_x = X[:,1]
...
def print_normalize_data(X, Y):
    theta = [[0.0]]* (X.shape[1])
    mlr = MLR(theta)
    norm_X = mlr.normalize(X)
    plt.scatter(X[:, 1], Y, color="green")
    plt.scatter(norm_X[:, 1], Y, color="red")
    plt.xlabel("x_axis")
    plt.ylabel("y_axis")
    plt.show()
...
def main():
    ...
    X_1 = add_intercept(np.array(data[:, :3]))
    print(f"X1 == {X_1}")
    Y_1 = transform_row_vector_to_column_vector(np.array(data[:, 3]))
    plot_basic_linear_regression(X_1, Y_1)
    # print_normalize_data(X_1, Y_1)
    return
